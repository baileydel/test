...
cd /d "%~dp0"
title GitHub Auto-Monitor Script
...
:setup_and_menu
REM Perform setup and cache status, display will be shown in menu
call :check_git_installed
call :setup_repository
call :setup_remote_if_needed
call :show_repository_status >nul 2>&1
...
REM Show cached repository status
call :show_cached_status
echo.
...
if not errorlevel 1 (
    ...
    if not errorlevel 1 (
        ...
        set "repo_status_line=Repository ready | Remote: !repo_name! | Branch: !current_branch!"
        set "repo_status_color=Green"
    ) else (
        ...
        set "repo_status_line=Repository error | Remote: !repo_name! (invalid) | Branch: !current_branch!"
        set "repo_status_color=Red"
    ...
    set "repo_status_line=Repository ready | Remote: not configured | Branch: !current_branch!"
    set "repo_status_color=Green"
)
goto :eof

:show_cached_status
REM Display cached repository status without checking
if defined repo_status_color (
    if "%repo_status_color%"=="Green" (
        powershell -Command "Write-Host 'Repository ' -NoNewline; Write-Host 'ready' -ForegroundColor Green -NoNewline; Write-Host ' | Remote: %repo_name% | Branch: %current_branch%'"
    ) else (
        powershell -Command "Write-Host 'Repository ' -NoNewline; Write-Host 'error' -ForegroundColor Red -NoNewline; Write-Host ' | Remote: %repo_name% (invalid) | Branch: %current_branch%'"
    )
) else (
    echo Repository status not yet determined
)
goto :eof
...
if not errorlevel 1 (
    ...
    if errorlevel 1 (
        ...
        set "remote_sync_status=new_repo"
        goto :eof
    )
    ...
    if errorlevel 1 (
        ...
        set "remote_sync_status=no_remote_branch"
        goto :eof
    )
